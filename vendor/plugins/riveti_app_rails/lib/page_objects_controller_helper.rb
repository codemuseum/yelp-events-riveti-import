...
#   include Riveti::PageObjectsControllerHelper
#
  ...
  module InstanceMethods
    ...
    protected
      ...
      def verify_request_signature
        ...
        sig_params = {}
        request.headers[Riveti::Constants.r_signature_headers_key].split('&').each { |pair| k,v = pair.split('='); sig_params[k] = v ? CGI::unescape(v) : v }
        
        raw = sig_params.reject { |k,v| k.to_s[0,7] != 'r_sig_' }.map{ |*args| args.join('=') }.sort.join('&')
        actual_sig = Digest::MD5.hexdigest([raw, Riveti::Constants.config['secret_key']].join)
        raise Riveti::IncorrectSignature if actual_sig != sig_params['r_sig']
        raise Riveti::SignatureTooOld if sig_params['r_sig_time'] && Time.at(sig_params['r_sig_time'].to_f) < Riveti::Constants.r_max_signature_age.ago
        true
      end
