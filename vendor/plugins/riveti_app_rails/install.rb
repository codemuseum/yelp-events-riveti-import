# Install a riveti_app.yml file into the local config directory.
# If a config file already exists, print a warning and exit.

src_config_file = File.join(File.dirname(__FILE__),'config', "riveti_app.yml.sample")
dest_config_file = File.join("config", "riveti_app.yml") 
...
if File::exists? dest_config_file
  ...
  puts "at www.riveti.com (for apps in development) and www.riveti.com (for production apps)."
end  
